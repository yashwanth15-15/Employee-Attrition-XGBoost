...
if not filtered_df[filtered_df["Attrition"] == "Yes"].empty:
    dept_rates = (
        ...
        .groupby(filtered_df["Department"])
        .mean()
...
gender_attr = (
    filtered_df.groupby(["Gender", "Attrition"]).size().reset_index(name="Count")
)
fig3 = px.bar(
    gender_attr,
    x="Gender",
...
ot_attr = (
    filtered_df.groupby(["OverTime", "Attrition"]).size().reset_index(name="Count")
)
fig4 = px.bar(
    ot_attr,
    x="OverTime",
...
if not filtered_df[filtered_df["Attrition"] == "Yes"].empty:
    ot_rates = (
        (filtered_df["Attrition"] == "Yes").groupby(filtered_df["OverTime"]).mean()
    )
...
fig5 = px.box(
    ...
    y="JobSatisfaction",
    color="Attrition",
    title="Job Satisfaction Levels by Attrition",
)
st.plotly_chart(fig5, width="stretch")

js_left = (
    filtered_df[filtered_df["Attrition"] == "Yes"]["JobSatisfaction"].mean()
    if not filtered_df[filtered_df["Attrition"] == "Yes"].empty
    else 0
)
js_stay = (
    filtered_df[filtered_df["Attrition"] == "No"]["JobSatisfaction"].mean()
    if not filtered_df[filtered_df["Attrition"] == "No"].empty
    else 0
)
...
fig6 = px.histogram(
    ...
    marginal="box",
    nbins=30,
    title="Monthly Income Spread and Attrition",
)
st.plotly_chart(fig6, width="stretch")

income_left = (
    filtered_df[filtered_df["Attrition"] == "Yes"]["MonthlyIncome"].mean()
    if not filtered_df[filtered_df["Attrition"] == "Yes"].empty
    else 0
)
income_stay = (
    filtered_df[filtered_df["Attrition"] == "No"]["MonthlyIncome"].mean()
    if not filtered_df[filtered_df["Attrition"] == "No"].empty
    else 0
)
...
fig7 = px.histogram(
    ...
    marginal="box",
    nbins=30,
    title="Age Demographics and Attrition",
)
st.plotly_chart(fig7, width="stretch")

age_left = (
    filtered_df[filtered_df["Attrition"] == "Yes"]["Age"].mean()
    if not filtered_df[filtered_df["Attrition"] == "Yes"].empty
    else 0
)
age_stay = (
    filtered_df[filtered_df["Attrition"] == "No"]["Age"].mean()
    if not filtered_df[filtered_df["Attrition"] == "No"].empty
    else 0
)
...
fig8 = px.bar(
    ...
    color="Attrition",
    barmode="group",
    title="Work Life Balance Rating (1=Bad to 4=Excellent)",
)
...
if not filtered_df[filtered_df["Attrition"] == "Yes"].empty:
    wlb_rates = (
        (filtered_df["Attrition"] == "Yes")
        .groupby(filtered_df["WorkLifeBalance"])
        .mean()
    )
...
if numeric_df.shape[1] > 1:
    ...
    st.dataframe(attr_corr.style.background_gradient(cmap="RdBu_r"), width="stretch")

    st.info(
        "Note: Features with zero variance (constant values) are automatically excluded because their correlation with the target is statistically undefined."
    )
...
with col_csv:
    ...
    st.download_button(
        ...
        data=csv_data,
        file_name=f"employee_attrition_eda_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
    )
...
with col_excel:
    ...
    st.download_button(
        ...
        file_name=f"employee_attrition_eda_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
