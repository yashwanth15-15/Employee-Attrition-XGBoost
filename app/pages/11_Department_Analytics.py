...
st.markdown(
    "Enterprise HR analytics tracking real-time attrition risk across all departments."
)
...
with st.spinner("Loading Department Analytics Data..."):
    raw_df = load_data()

if raw_df.empty:
    st.info(
        "ℹ️ No historical data available. Generate employee predictions to unlock analytics."
    )
    st.stop()
...
highest_risk_dep = (
    dep_risk_avg.sort_values("prediction_probability", ascending=False).iloc[0][
        "department"
    ]
    if not dep_risk_avg.empty
    else "N/A"
)
lowest_risk_dep = (
    dep_risk_avg.sort_values("prediction_probability", ascending=True).iloc[0][
        "department"
    ]
    if not dep_risk_avg.empty
    else "N/A"
...
high_risk_df = df[df["risk_category"] == "High"].sort_values(
    "prediction_probability", ascending=False
)
if not high_risk_df.empty:
    ...
    st.dataframe(high_risk_table.head(10), width="stretch", hide_index=True)
else:
    st.success(
        "No high-risk employees found in the current filtered dataset. Excellent retention health!"
    )
    ...
    with st.container(border=True):
        ...
        if high_risk_exposure == 0:
            ...
            if highest_cost_dept is not None:
                ...
                st.metric(
                    f"Exposure in {highest_cost_dept['department']}",
                    f"₹{highest_cost_dept['Total_Replacement_Cost']:,.2f}",
                )
...
with b_col2:
    with st.container(border=True):
        ...
        if len(departments) == 1:
            insights.append(
                "Current analytics include one department only. Department-level benchmarking will become available as additional departments are predicted."
            )
        else:
            if highest_risk_dep != "N/A":
                insights.append(
                    f"**{highest_risk_dep}** currently holds the highest average attrition risk ({dept_summary.iloc[0]['Average_Risk']:.1f}%)."
                )
            if lowest_risk_dep != "N/A":
                insights.append(
                    f"**{lowest_risk_dep}** maintains the strongest retention outlook across the organization."
                )
        ...
        if not dept_summary.empty and len(departments) > 1:
            ...
            if lowest_wlb["Avg_WLB"] < 2.5:
                insights.append(
                    f"Average Work-Life Balance in **{lowest_wlb['department']}**: {lowest_wlb['Avg_WLB']:.1f} (Below Organizational Target)"
                )

        if len(insights) == 0 or total_emp == 0:
            insights.append(
                "Current workforce metrics are within expected organizational thresholds."
            )
...
if high_risk == 0:
    ...
    with e_col3:
        with st.container(border=True):
            ...
            if lowest_risk_dep != "N/A":
                st.write(
                    f"✔ Replicate retention success of {lowest_risk_dep} across org"
                )
...
with ex_col1:
    st.download_button(
        "📄 Download CSV",
        data=csv_data,
        file_name=f"employee_attrition_department_analytics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
        width="stretch",
    )
...
with ex_col2:
    st.download_button(
        "📜 Download JSON",
        data=json_data,
        file_name=f"employee_attrition_department_analytics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
...
def generate_dept_pdf(dept_summary_df, insights_text):
    ...
    # Cover Section
    elements.append(
        Paragraph("Enterprise HR Department Analytics Summary", title_style)
    )
    elements.append(Spacer(1, 12))
    elements.append(
        Paragraph(
            "Executive insights detailing attrition risk and financial exposure across the organization.",
            normal_style,
        )
    )
...
try:
    ...
    with ex_col3:
        st.download_button(
            ...
            data=pdf_bytes,
            file_name=f"employee_attrition_department_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
