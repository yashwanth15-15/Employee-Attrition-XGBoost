...
with st.spinner("Computing SHAP values..."):
    X_test, X_test_display, shap_values, base_value = load_and_compute_shap(encoders, features, explainer)
...
st.markdown("Global explanations show how the model behaves overall across all employees.")
...
with col_g1:
    ...
    shap.summary_plot(shap_values, X_test, plot_type="dot", show=False, feature_names=features)
    st.pyplot(fig)
    plt.close()
    st.info("**How to read:** Each dot is an employee. Red indicates a high feature value, blue indicates a low value. Dots to the right push the model toward Attrition (Yes).")
...
with col_g2:
    ...
    fig_bar = px.bar(top_shap, x="Mean |SHAP|", y="Feature", orientation='h', title="Top 15 Features by Mean |SHAP|")
    st.plotly_chart(fig_bar, use_container_width=True)
    st.info("**Why it matters:** Unlike standard XGBoost importance, SHAP importance measures the average absolute impact each feature has on the final predicted probability.")
...
with col_d2:
    ...
    dep_df = pd.DataFrame({
        'Feature Value': X_test[dep_feat],
        'SHAP Value': shap_values[:, feat_idx]
    })
    try:
        fig_dep = px.scatter(dep_df, x='Feature Value', y='SHAP Value', trendline="lowess", title=f"Dependence Plot for {dep_feat}")
    except (ImportError, ModuleNotFoundError):
        fig_dep = px.scatter(dep_df, x='Feature Value', y='SHAP Value', title=f"Dependence Plot for {dep_feat}")
        st.info("LOWESS trendline unavailable because the optional 'statsmodels' package is not installed.")
    fig_dep.add_hline(y=0, line_dash="dash", line_color="gray")
    st.plotly_chart(fig_dep, use_container_width=True)
    st.info(f"**Interpretation:** Shows how varying **{dep_feat}** directly impacts the prediction. Points above 0 increase attrition risk; points below 0 decrease it.")
...
st.markdown("Select a specific employee to see exactly why the model predicted they will stay or leave.")
...
loc_df = pd.DataFrame({
    'Feature': features,
    'Value': loc_x_display.values,
    'SHAP Value': loc_shap
})
loc_df['Contribution Direction'] = loc_df['SHAP Value'].apply(lambda x: 'Increases Risk 📈' if x > 0 else 'Lowers Risk 📉')
loc_df['Absolute SHAP'] = np.abs(loc_df['SHAP Value'])
loc_df = loc_df.sort_values(by='Absolute SHAP', ascending=False).drop(columns=['Absolute SHAP'])

# Employee KPIs
st.markdown(f"#### 🎯 Prediction for Employee **{selected_emp_idx}**: **{pred_class}** (Probability: {prob*100:.1f}%)")
...
with col_l1:
    ...
    if len(loc_df) > 10:
        rest_row = pd.DataFrame([{'Feature': 'All Other Features', 'Value': '-', 'SHAP Value': rest_shap}])
    ...
    fig_wf = go.Figure(go.Waterfall(
        name="Employee Breakdown", orientation="h",
        measure=measure,
        y=wf_df['Feature'],
        x=wf_df['SHAP Value'],
    ...
    fig_wf.update_layout(title="Feature Contributions to Prediction Log-Odds", waterfallgap=0.3)
    st.plotly_chart(fig_wf, use_container_width=True)

# 6. Local Explanation Table
with col_l2:
    st.subheader("Local Explanation Table")
    st.dataframe(loc_df[['Feature', 'Value', 'SHAP Value', 'Contribution Direction']].head(12), hide_index=True, use_container_width=True)
...
with c_pos:
    ...
    for _, row in top_pos.iterrows():
        st.write("- " + get_business_interpretation(row['Feature'], row['SHAP Value'], row['Value']))
...
with c_exp1:
    csv_global = pd.DataFrame(shap_values, columns=features).to_csv(index=False).encode('utf-8')
    st.download_button(
        label="Download Global SHAP Matrix (CSV)",
        data=csv_global,
        file_name="global_shap_values.csv",
        mime="text/csv"
    )
...
with c_exp2:
    ...
    st.download_button(
        ...
        data=csv_local,
        file_name=f"local_shap_emp_{selected_emp_idx}.csv",
        mime="text/csv"
    )

st.caption("Generated by Employee Attrition Prediction System | Dynamic SHAP Explainability Module")