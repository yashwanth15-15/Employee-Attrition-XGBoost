...
X_train, X_test, y_train, y_test, total_samples = load_and_preprocess_test_data(
    encoders, features
)
...
with col_v1:
    ...
    st.info(
        f"**Business Impact:** The model caught {tp} employees likely to leave (True Positives) but missed {fn} (False Negatives)."
    )
...
with col_v2:
    ...
    st.dataframe(
        df_report.style.highlight_max(
            axis=0, subset=["precision", "recall", "f1-score"], color="lightgreen"
        ),
        width="stretch",
    )
    st.info(
        "**Explanation:** Shows class-level breakdown. '1' represents Attrition (Yes), '0' represents Retention (No)."
    )
...
with col_v3:
    ...
    fig_roc.update_layout(
        title=f"Receiver Operating Characteristic (AUC: {roc_auc:.3f})",
        xaxis_title="False Positive Rate",
        yaxis_title="True Positive Rate",
    )
    st.plotly_chart(fig_roc, width="stretch")
    st.info(
        "**Interpretation:** A higher AUC (closer to 1.0) means the model is excellent at distinguishing between employees who stay and those who leave."
    )
...
with col_v4:
    ...
    st.info(
        "**Why it matters:** Because attrition is a minority class, the PR curve provides a better assessment of performance than ROC by focusing purely on the positive class."
    )
...
with col_v5:
    ...
    df_prob = pd.DataFrame(
        {
            "Probability": y_prob,
            "Actual": ["Attrition" if y == 1 else "No Attrition" for y in y_test],
        }
    )
...
if prec > 0.7:
    weakness_txt = (
        f"High Precision ({prec*100:.1f}%), indicating very few false alarms."
    )
else:
...
st.success(
    f"**💪 Strengths:** {strength_txt} Additionally, a high ROC-AUC ({roc_auc:.3f}) shows strong separability."
)
st.warning(f"**⚠️ Limitations:** {weakness_txt}")
st.info(
    f"**💼 Business Impact:** By catching {tp} out of {tp+fn} potential departures in this test set, HR can proactively intervene and potentially save ₹{tp * 50000:,} (assuming a conservative ₹50k replacement cost per employee)."
)
...
with col_adv:
    ...
    st.dataframe(adv_df, hide_index=True, width="stretch")
...
with col_info:
    ...
    n_estimators = (
        hyperparams.get("n_estimators", "N/A")
        if isinstance(hyperparams, dict)
        else "N/A"
    )
    max_depth = (
        hyperparams.get("max_depth", "N/A") if isinstance(hyperparams, dict) else "N/A"
    )
...
with c_exp1:
    ...
    st.download_button(
        ...
        data=csv_report,
        file_name=f"employee_attrition_classification_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
    )
...
with c_exp2:
    ...
    st.download_button(
        ...
        data=csv_metrics,
        file_name=f"employee_attrition_performance_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        mime="text/csv",
    )

st.caption(
    "Generated by Employee Attrition Prediction System | Dynamic Model Evaluation Module"
)
